...
export default async (req, res, userSuppliedOptions) => {
  ...
  return new Promise(async (resolve) => {
    ...
    const secret =
      userSuppliedOptions.secret ||
      createHash('sha256')
        .update(JSON.stringify({ baseUrl, basePath, ...userSuppliedOptions }))
        .digest('hex');
    ...
    const useSecureCookies =
      userSuppliedOptions.useSecureCookies || baseUrl.startsWith('https://');
    ...
    const cookies = {
      ...
      tokensecret: {
        name: `${useSecureCookies ? '__Host-' : ''}next-auth.tokensecret`,
        options: {
          sameSite: 'lax',
          path: '/',
          secure: useSecureCookies,
    ...
    const redirect = (redirectUrl, secret) => {
      const reponseAsJson = !!(req.body && req.body.json === 'true');

      if (reponseAsJson) {
        cookie.set(
          res,
          cookies.tokensecret.name,
          secret,
          cookies.tokensecret.options
        );
    ...
    if (req.method === 'GET') {
      switch (action) {
        ...
        case 'signin':
          if (options.pages.signIn) {
            let redirectUrl = `${options.pages.signIn}${
              options.pages.signIn.includes('?') ? '&' : '?'
            }callbackUrl=${options.callbackUrl}`;
            if (req.query.error) {
              redirectUrl = `${redirectUrl}&error=${req.query.error}`;
            }
        ...
        case 'signout':
          if (options.pages.signOut) {
            return redirect(
              `${options.pages.signOut}${
                options.pages.signOut.includes('?') ? '&' : '?'
              }error=${error}`
            );
          }
          ...
          pages.render(
            ...
            res,
            'signout',
            { baseUrl, basePath, csrfToken, callbackUrl: options.callbackUrl },
            done
          );
          break;
        case 'callback':
          if (provider && options.providers[provider]) {
            console.log('server2', req.query);
      ...
      switch (action) {
        ...
        case 'callback':
          if (provider && options.providers[provider]) {
            ...
            if (
              options.providers[provider].type === 'credentials' &&
              !csrfTokenVerified
            ) {
              return redirect(`${baseUrl}${basePath}/signin?csrf=true`);
            }
      ...
      res
        .status(400)
        .end(`Error: HTTP ${req.method} is not supported for ${url}`);
      return done();
    }
  });
};
