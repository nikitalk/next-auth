        ...
        if (provider.idToken) {
          ...
          _decodeToken(
            ...
            async (error, profileData) => {
              const { profile, account, OAuthProfile } = await _getProfile(
                error,
                profileData,
                accessToken,
                refreshToken,
          ...
          client.get(provider, accessToken, async (error, profileData) => {
            const { profile, account, OAuthProfile } = await _getProfile(
              error,
              profileData,
              accessToken,
...
async function _getProfile(
  ...
  try {
    ...
    if (provider.getEmailFromRawResponse) {
      profileData.response[0].email = results.email;
      const q = profileData.response[0];
      profileData = q;
    }
...
async function _getOAuthAccessToken(code, provider, callback) {
  ...
  const setGetAccessTokenAuthHeader =
    provider.setGetAccessTokenAuthHeader !== null
      ? provider.setGetAccessTokenAuthHeader
      : true;
  ...
  if (!params.client_secret) {
    ...
    if (provider.clientSecretCallback) {
      params.client_secret = await provider.clientSecretCallback(
